...
export default function AdminClassSchedulePage({
  ...
  const handleClassSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const list = JSON.parse(JSON.stringify(weeklyClasses));
      ...
      api.saveSchedule("class", 2026, list)
        .then(() => {
        ...
        .catch((err) => {
          console.error("Failed to save class schedule to DB", err);
  ...
  const handleClassDelete = (dayKey: string, timeSlot: string) => {
    if (confirm(isTh ? "คุณต้องการลบวิชาในคาบเวลานี้ใช่หรือไม่?" : "Are you sure you want to clear this class slot?")) {
      ...
      api.saveSchedule("class", 2026, list)
        ...
        .catch((err) => {
          console.error("Failed to delete class schedule from DB", err);
